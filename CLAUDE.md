# CLAUDE.md — mosaic-module-habits
...
|-------|-------|
| **Slug** | `habits` |
| **Package** | `mosaic-module-habits` |
| **Purpose** | Habits tracker module for Mosaic — daily and weekly habit tracking with streaks |
| **Framework** | `/opt/CODE/mosaic-framework` |
| **API mount** | `/api/habits/` |
| **DB prefix** | `habits_` (e.g., `habits_habits`) |
...
export default {
  ...
  name: 'Habits',
  version: '1.x.x',
  router,          // Express Router — mounted at /api/habits/
...
- Table names: `habits_<table>` (e.g., `habits_habits`, `habits_completions`)
- Migration pattern: inline in `migrate(db)`:
  - New tables: `CREATE TABLE IF NOT EXISTS`
  - New columns: check with `db.pragma('table_info(habits_<table>)')`, then `ALTER TABLE ADD COLUMN`
